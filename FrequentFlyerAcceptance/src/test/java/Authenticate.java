    ...
    public Authenticate() {
        And("^\"([^\"]*)\" has registered online with a password of \"([^\"]*)\"$", (String member, String password) -> {
            responseEntity = restTemplate.getForEntity(
                    createURL("/member?name="+member+"&password="+password),
                    String.class);
        });
        ...
        Given("^the account has expired$", () -> {
            assertThat(responseEntity.getBody(), containsString("Account has expired"));
        });
        Then("^he should be informed that his account has expired$", () -> {
            assertThat(responseEntity.getBody(), containsString("Account has expired"));
        });
        And("^he should be invited to renew his account$", () -> {
            assertThat(responseEntity.getBody(), containsString("Please renew your account"));
        });
    }
}
